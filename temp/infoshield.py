...
if __name__ == '__main__':
    ...
    #coarse.clustering()
    #ccs = [len(cc) for cc in nx.connected_components(coarse.cluster_graph) if len(cc) > 1]
    #print(sorted(ccs, reverse=True)[:10])
    #coarse.print_clusters()
    #post_process_coarse(sys.argv[1].split('.')[0])
    run_infoshieldfine(sys.argv[1].split('.')[0] + '_LSH_labels.csv')
    #tfinal = time.time() - t
    #with open('times.txt', 'a') as f:
    #    f.write('{}\t{}'.format(sys.argv[1], tfinal))
    if len(sys.argv) == 3:
